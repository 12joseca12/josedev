...
Task 3: complete (commit 528bba5, branch fase-3c-comisiones-ui off main). `src/services/commissions-api.ts`: `listCommissions` (enumerated cols, RLS admin-only scope) + `markCommissionPaid` (pending→paid, `.select("id")` for 0-row detection → "not-updatable" on RLS/estado miss, P3-8). Mirrors leads-api (FetchResult, browser client). typecheck+lint clean (7 lint errors are all pre-existing in unrelated files).
Task 4: complete (commit 2f552b5). `/admin/finanzas` page + `finanzas-client.tsx` + `use-finanzas-data.ts` (fetches commissions+staff+leads in parallel, names resolved client-side) + nav link in dash-shell + `finanzas.*` literals es/en. Per-closer summary (earned=total_ganado, paid/pending via summarizeByCloser) + per-project ledger table with mark-paid button (pending only, in-flight guard, toast). dash-* tokens, responsive table/cards, a11y aria-labels. typecheck + 148 tests + build (route compiles es/en) green.
LIVE VERIFICATION (Step 6): done via a rollback-wrapped DO block against prod (nrgrmymsjtgayzejtawa, essentially empty: 1 staff/1 auth user/0 leads/0 entries). 5/5 trigger scenarios PASS — [1] close→pending entry(100.00)+total_ganado; [2] mark-paid→paid+paid_at, total_ganado unchanged; [3] revert-pending→reversed+total_ganado down, re-close→NEW pending (2 entries, partial-unique allows); [4] revert-PAID→BLOCKED (money-safety gate); [5] no-comision→no entry. Zero rows persisted (RAISE-forced rollback; post-check confirms 0 leftover test users). RLS scenario 6 verified by pg_policies: SELECT + UPDATE both admin-only, UPDATE USING estado='pending' / WITH CHECK estado='paid' AND paid_at IS NOT NULL → non-admin gets 0 rows ("not-updatable"). get_advisors: only 2 pre-existing WARNs (pg_net-in-public, leaked-password-protection), none new.
3c CODE-COMPLETE + live-verified. Merged to LOCAL main (merge commit e6cf4d2, --no-ff). NOT pushed to origin yet (user's call). DEFERRED (P3, before real payouts): column-freeze trigger on mark-paid. USER manual: authed UI smoke of /admin/finanzas (needs admin session + a closer with comision + a closed lead — prod has none yet).
